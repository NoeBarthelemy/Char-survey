...
```{r}
Survey_Dummy_Variables_Rowan_V15 <- Survey_Dummy_Variables_Rowan_V14 %>% 
...
The idea is to combine our CRRA and Aloocation variables in order to see if they give more information within a regression model. 
Let's try to see how we can combine these things in a meaningful way.

The objective is to normalise the CRRA values to include them into a regression.

```{r}
Survey_Dummy_Variables_Rowan_Simple_V2 <- Survey_Dummy_Variables_Rowan_Simple_V1 %>% 
  mutate(., Hybrid_CRRA_Alloc = Pop_CRRA * Alloc_HR_Round2)
```
